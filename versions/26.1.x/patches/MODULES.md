...
| `minecraft-0017-Linear-save-drain` | `LinearFlushCoordinator.flushAllDirty(force)` + `evictAll()`; hooks in `ChunkMap.processUnloads` (age-gated, every unload pass) and `RegionShutdownThread.run()` post-`stopServer` (stop drain); earlier `ServerChunkCache.save` / `ServerLevel.close` / `saveAllChunks` / `saveIncrementally` hooks kept as backups |
