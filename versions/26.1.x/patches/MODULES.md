...
| `minecraft-0014-Linear-default-format-9` | Shipped defaults ANVIL→LINEAR, level 1→9 (`WorldConfiguration`, `DEFAULT_COMPRESSION_LEVEL`, config fallback, conversion level) |
...
| `minecraft-0017-Linear-save-drain` | `LinearFlushCoordinator.flushAllDirty(force)` + `evictAll()`; hooks in `ChunkMap.processUnloads` (per-tick unload drain removed; non-force drains gated to at most one per flush-frequency window) and `RegionShutdownThread.run()` post-`stopServer` (stop drain); earlier `ServerChunkCache.save` / `ServerLevel.close` / `saveAllChunks` / `saveIncrementally` hooks kept as backups |
...
| `paper-0010-Linear-default-format-9` | Paper-side defaults flip (config default + fallback 1→9); carries `LinearDefaultFormatTest` |
